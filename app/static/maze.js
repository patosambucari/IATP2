document.addEventListener("DOMContentLoaded", function() {
  ...
  form.addEventListener("submit", function(event) {
    ...
    const algorithm = event.submitter.id.split("-")[1];
    if (algorithm === "bfs") {
      solveBFS();
  ...
  document.getElementById("update-maze").addEventListener("click", function() {
    const rows = parseInt(document.getElementById("rows").value);
    const cols = parseInt(document.getElementById("cols").value);
    const start = document.getElementById("start").value;
    const end = document.getElementById("end").value;
    ...
    canvas.width = cols * 50;
    canvas.height = rows * 50;
  
    // Actualizar la variable global "rows" y "cols"
    window.rows = rows;
    window.cols = cols;
  
    // Actualizar el dibujo del laberinto
    draw();
  
    // Actualizar la posición de inicio y final en el formulario
